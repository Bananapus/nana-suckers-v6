...
- **Do not send ETH on enable-only `mapTokens()` calls.** `msg.value` is only consumed when one or more mappings are being disabled and need transport payment for a final root flush. On enable-only batches, any ETH sent is not refunded.
