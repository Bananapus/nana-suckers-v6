...
- **`toRemoteFee`** is an ETH fee (in wei) paid into the project via `terminal.pay()` on each `toRemote()` call, making spam economically costly. The caller receives project tokens in return (incentivizing relayers). A "nothing to send" guard also prevents free repeated calls when nothing has changed.
...
- The [`JBTokenMapping`](src/structs/JBTokenMapping.sol) maps local mainnet ETH to remote Optimism ETH.
  ...
  - `toRemoteFee` prevents spam -- the caller pays 0.025 ETH into the project on each `toRemote()` call, receiving project tokens in return.
