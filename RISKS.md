...
- **Registry aggregate views fail open.** `JBSuckerRegistry.remoteBalanceOf`, `remoteSurplusOf`, and `remoteTotalSupplyOf` dedupe same-peer active suckers by the freshest accepted snapshot because each sucker caches the entire remote chain's state — multiple active suckers for the same chain report redundant snapshots, not additive shares. MAX is only a same-freshness tie-breaker and a deprecated-sucker fallback when no active sucker answers for that peer chain. The views silently skip any sucker that reverts. This preserves liveness for dashboards and cross-chain estimates, but it means the returned aggregate can understate remote state whenever one peer is broken, censored, or simply expensive to query.
