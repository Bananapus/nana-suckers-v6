...
A sucker is an oracle-free data carrier. On receive, `fromRemote` rebuilds an enumerable per-currency context set: each remote context resolves to its local token, whose authoritative accounting-context currency is read once from the terminal (`accountingContextForTokenOf(token).currency`) and cached. A project may set that accounting-context currency to a well-known id such as USD, not just the `uint32(uint160(token))` convention, so the currency a context carries is whatever the local terminal declares. Same-currency contexts within a snapshot are summed, and a fresher snapshot rebuilds the whole set (dropped contexts simply vanish). The sucker exposes one raw view, `peerChainContextsOf()`, returning `JBPeerChainContext{currency, decimals, surplus, balance}` per currency — un-valued amounts in each currency's own units. The sucker holds no prices/oracle reference.
