...
contract DeprecatedSuckerAggregateViewsTest is Test {
    ...
    function setUp() public {
        ...
        suckerA = DeprecatedViewMockSucker(
            ...
            payable(address(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("dedup-sucker-a")))))
        );
        ...
        suckerB = DeprecatedViewMockSucker(
            // forge-lint: disable-next-line(unsafe-typecast)
            payable(address(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("dedup-sucker-b")))))
        );
        ...
        // forge-lint: disable-next-line(unsafe-typecast)
        registry.deploySuckersFor({projectId: PROJECT_ID, salt: keccak256(bytes("dedup")), configurations: configs});
    }
