...
contract RegressionSwapHarness is JBSwapCCIPSucker {
    constructor(
...
contract RegressionSwapQueueOrderTest is Test {
    ...
    ERC20Mock usdc;
    ERC20Mock weth;
    RegressionMockTerminal terminal;
    RegressionSwapHarness sucker;
    ...
    function setUp() public {
        ...
        RegressionSwapHarness singleton = new RegressionSwapHarness(
        ...
        sucker = RegressionSwapHarness(
            payable(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("regression"))))
        );
        sucker.initialize(PROJECT_ID);
    }

    /// @notice out-of-order claims now get correct rates.
    ...
    /// @notice out-of-order root arrival doesn't affect rate application.
    ...
    /// @notice missing nonce only blocks claims for leaves NOT in any received range.
