...
contract RegressionCCIPHarness is JBCCIPSucker {
    constructor(
...
contract RegressionCCIPWrappedNativeMisunwrapTest is Test {
    ...
    RegressionCCIPHarness sucker;
    RegressionMockWETH weth;

    function setUp() public {
        weth = new RegressionMockWETH();
        ...
        RegressionCCIPHarness singleton = new RegressionCCIPHarness(
            JBCCIPSuckerDeployer(DEPLOYER), IJBDirectory(DIRECTORY), IJBTokens(TOKENS), IJBPermissions(PERMISSIONS)
        );
        // forge-lint: disable-next-line(unsafe-typecast)
        sucker = RegressionCCIPHarness(
            payable(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("regression-ccip-weth"))))
        );
