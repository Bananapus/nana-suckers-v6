...
contract RegressionMapTokensHarness is JBSucker {
    constructor(
...
contract RegressionMapTokensEnableOnlyValueStuckTest is Test {
    ...
    function test_mapTokensEnableOnlyBatchRefundsMsgValue() external {
        ...
        RegressionMapTokensHarness sucker = RegressionMapTokensHarness(
            ...
            payable(address(
                    LibClone.cloneDeterministic(address(singleton), keccak256(bytes("regression-enable-only-msgvalue")))
                ))
        );
