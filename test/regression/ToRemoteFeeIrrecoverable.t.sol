...
contract RegressionFeeIrrecoverableHarness is JBSucker {
...
contract RegressionTerminalStub {
...
contract RegressionToRemoteFeeIrrecoverableTest is Test {
    ...
    RegressionFeeIrrecoverableHarness internal sucker;
    RegressionTerminalStub internal terminal;

    function setUp() public {
        terminal = new RegressionTerminalStub();

        // Mock DIRECTORY.PROJECTS() so the JBSucker constructor can initialize the PROJECTS immutable.
        vm.mockCall(DIRECTORY, abi.encodeCall(IJBDirectory.PROJECTS, ()), abi.encode(PROJECT));

        RegressionFeeIrrecoverableHarness singleton = new RegressionFeeIrrecoverableHarness({
        ...
        sucker = RegressionFeeIrrecoverableHarness(
            // forge-lint: disable-next-line(unsafe-typecast)
            payable(address(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("regression-fee-stuck")))))
        );
