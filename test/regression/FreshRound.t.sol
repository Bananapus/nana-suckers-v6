...
contract RegressionMockPoolManager is IPoolManager {
...
contract RegressionFreshRoundTest is Test {
    ...
    function test_retrySwapZeroOutputClearsGateAndAllowsZeroBackedClaims() public {
        ...
        deployer.setSwapConstants(
            ...
            IPoolManager(address(new RegressionMockPoolManager())),
        ...
        RegressionSwapHarness(payable(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("retry")))));
    ...
    function test_peerTopologyDriftBreaksDefaultPeerAuthentication() public {
        ...
        // forge-lint: disable-next-line(unsafe-typecast)
        JBCCIPSucker(payable(address(deployerA.createForSender(PROJECT_ID, keccak256(bytes("peer")), bytes32(0)))));
        ...
        Client.Any2EVMMessage memory message = Client.Any2EVMMessage({
            // forge-lint: disable-next-line(unsafe-typecast)
            messageId: keccak256(bytes("peer")),
            sourceChainSelector: REMOTE_SELECTOR,
