...
contract RegressionSwapZeroBatchMockTerminal {
...
contract RegressionSwapZeroBatchHarness is JBSwapCCIPSucker {
    constructor(
...
contract RegressionSwapZeroAmountBatchGapTest is Test {
    ...
    RegressionSwapZeroBatchMockTerminal internal terminal;
    RegressionSwapZeroBatchHarness internal sucker;

    function setUp() public {
        usdc = new ERC20Mock("USDC", "USDC", address(this), 0);
        weth = new ERC20Mock("WETH", "WETH", address(this), 0);
        terminal = new RegressionSwapZeroBatchMockTerminal();
        ...
        RegressionSwapZeroBatchHarness singleton = new RegressionSwapZeroBatchHarness(
        ...
        sucker = RegressionSwapZeroBatchHarness(
            payable(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("zero"))))
        );
    ...
    function test_zeroAmountBatch_doesNotBlockLaterClaims() external {
        ...
        Client.Any2EVMMessage memory zeroBatchMessage = Client.Any2EVMMessage({
            // forge-lint: disable-next-line(unsafe-typecast)
            messageId: keccak256(bytes("zero")),
        ...
        Client.Any2EVMMessage memory fundedBatchMessage = Client.Any2EVMMessage({
            // forge-lint: disable-next-line(unsafe-typecast)
            messageId: keccak256(bytes("funded")),
            sourceChainSelector: REMOTE_CHAIN_SELECTOR,
            sender: abi.encode(address(sucker)),
            data: abi.encode(
                uint8(0),
