...
contract RegressionSwapZeroLocalTerminal {
...
contract RegressionSwapZeroLocalController {
...
contract RegressionSwapZeroLocalHarness is JBSwapCCIPSucker {
    constructor(
...
contract RegressionSwapZeroLocalTotalUnbackedClaimTest is Test {
    ...
    RegressionSwapZeroLocalTerminal internal terminal;
    RegressionSwapZeroLocalController internal controller;
    RegressionSwapZeroLocalHarness internal sucker;

    function setUp() public {
        usdc = new ERC20Mock("USDC", "USDC", address(this), 0);
        weth = new ERC20Mock("WETH", "WETH", address(this), 0);
        terminal = new RegressionSwapZeroLocalTerminal();
        controller = new RegressionSwapZeroLocalController();
        ...
        RegressionSwapZeroLocalHarness singleton = new RegressionSwapZeroLocalHarness(
        ...
        sucker = RegressionSwapZeroLocalHarness(
            // forge-lint: disable-next-line(unsafe-typecast)
            payable(LibClone.cloneDeterministic(address(singleton), keccak256(bytes("regression-zero-local"))))
        );
