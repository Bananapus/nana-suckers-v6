...
contract RegressionGapSucker is JBSucker {
...
/// @notice Tests for regression gaps: (1) concurrent sucker prepare/claim operations happening
...
contract TestRegressionGaps is Test {
    ...
    function setUp() public {
        ...
        sucker = _createTestSucker(PROJECT_ID, "regression_gap_salt");
    ...
    function _createTestSucker(uint256 projectId, bytes32 salt) internal returns (RegressionGapSucker) {
        RegressionGapSucker singleton =
            new RegressionGapSucker(IJBDirectory(DIRECTORY), IJBPermissions(PERMISSIONS), IJBTokens(TOKENS), FORWARDER);

        RegressionGapSucker clone =
            RegressionGapSucker(payable(address(LibClone.cloneDeterministic(address(singleton), salt))));
