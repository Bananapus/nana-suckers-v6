...
contract HookedFallbackPoolManager {
...
contract RevertingGeomeanOracleHook is IGeomeanOracle {
    function observe(PoolKey memory, uint32[] memory)
        external
        pure
        override
...
contract HookedV4SpotFallbackOverrideTest is Test {
    ...
    HookedFallbackPoolManager internal poolManager;
    RevertingGeomeanOracleHook internal hook;

    function setUp() external {
        vm.warp(1000);

        harness = new HookedFallbackHarness();
        poolManager = new HookedFallbackPoolManager();
        hook = new RevertingGeomeanOracleHook();
    ...
    function test_hookedV4WithBrokenOracleCannotFallbackToSpotAndOverrideLiveV3() external {
        ...
        assertFalse(isV4, "broken hooked V4 should not fall back to spot over live V3 TWAP");
