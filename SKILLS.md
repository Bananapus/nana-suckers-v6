...
| Contract | Role |
|----------|------|
| `JBSucker` | Abstract base with full lifecycle: prepare, toRemote, fromRemote, claim, emergency hatch, deprecation. Manages outbox/inbox merkle trees per terminal token. Uses `ERC2771Context` for meta-transactions. Deployed as minimal clones via `Initializable`. Has immutable `FEE_PROJECT_ID` (typically project ID 1) and `TO_REMOTE_FEE` (ETH fee in wei) set at construction — uniform across all tokens, non-bypassable by integrators. |
...
| Name | Value | Context |
|------|-------|---------|
| `FEE_PROJECT_ID` | Set at construction (typically `1`) | The project that receives `TO_REMOTE_FEE` payments via `terminal.pay()`. Immutable. |
| `TO_REMOTE_FEE` | Set at construction (e.g. `0.001 ether`) | ETH fee (in wei) paid into the fee project on each `toRemote()` call. Contract-level immutable — uniform across all tokens, non-bypassable by integrators. |
